...
I just believe that Gatsby is the perfect platform for my blog and I want to start writing! How can I do that, like, yesterday?
...
However, I'd love if this starter theme had support for some extra features like:

1. Display in the user's preferred color scheme by default (light theme or dark theme)
2. Click images to expand them to a lightbox
3. Add pages that are not listed in the blog index, but pinned in a top menu
4. Add a top level layout component to be applied in place of the default theme; although this is actually a "theme", it seems that its purpose is more for quickstarting a blog than for styling purposes, so, allowing the layout to be customized could make sense

With these features (especially 1 through 3), I think I could stick with this theme for a pretty long time! I have more ideas that would make it even more appealing, but then it would start sounding like a "create-react-app" for Gatsby - the scope would be so big that I'd have to create my own Gatsby setup from scratch. I'll get to that point in some months!
